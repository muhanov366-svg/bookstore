app.post('/api/login', async (req, res) => {
  ...
  console.log('Password:', password ? '✓ Получен' : '✗ НЕ ПОЛУЧЕН');
  ...
  try {
    ...
    console.log('Результат запроса:', JSON.stringify(result.rows));
...
// Добавьте этот маршрут после health check
...
async function initDatabase() {
  try {
    ...
    `);
    console.log('✅ Таблица rentals создана');
    ...
    if (booksCount.rows[0].count === '0') {
      ...
      `);
      console.log('✅ Начальные книги добавлены');
    }
...
// Получить все аренды (для админа)
